...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--heartbeat-timeout", type=float, default=240.0)
    parser.add_argument(
        "--location-settle-frames",
        type=int,
        default=60,
        help="frames to hold the selected location before accepting it",
    )
    parser.add_argument(
        "--loading-prompt-hold-frames",
        type=int,
        default=0,
        help="extra polls to leave the native loading prompt visible before accepting it",
    )
    parser.add_argument(
        "--weapon-start-kind",
        type=int,
...
def build_input_script(
    ...
    last_location_input = 1450 + max(0, slot - 1) * 30
    location_accept_frame = max(2020, last_location_input + location_settle_frames)
    lines.append(f"{location_accept_frame}+3=CROSS")
    ...
    character_select_frame = location_accept_frame + 200
    lines.extend(
        [
            ...
            f"{character_select_frame + 420 + loading_prompt_hold_frames}+3=CROSS",
            "[gameplay]",
            "",
        ]
    )
...
def run_one(
    ...
    env.update(
        {
            ...
            "RECOMPONE_SOAK_HEARTBEAT_FRAMES": os.environ.get(
    ...
    stage_captures = []
    for source in sorted(capture_source.glob("recompone_capture_*.ppm")):
        try:
            if source.stat().st_mtime < started - 1:
                continue
        except FileNotFoundError:
        ...
        target = output / f"{stem}.{source.name}"
        shutil.copy2(source, target)
        stage_captures.append(str(target))
...
def main() -> int:
    ...
    if args.location_settle_frames < 0:
