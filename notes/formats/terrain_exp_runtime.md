...
### N64 archive rows versus TMEM upload layout

Dreamland uses two different N64 texture-storage paths; treating them as one
format rule corrupts either the terrain atlas or the standalone road/models:

- `tools/n64_reference/audit_runtime_texture_layout.py` compares the decoded
  `320x128` CI8 terrain atlas with an original-game RDRAM capture.
- For the large `XBMP` terrain atlas, none of the 40 archived linear `32x32`
  tiles occurs in the runtime upload
  bank. All 40 occur consecutively after swapping the two 32-bit halves of
  every odd 64-bit row; tile zero begins at RDRAM `0x002DFA78`.
- The captured RDP stream loads that buffer with `LoadBlock`, `DXT=0`. This
  proves the CPU has already performed the odd-row repack before submission.
- Standalone XOBF and XRTP texture storage follows the opposite path. Eight
  independently matched records, including the `64x32` road texture at RDRAM
  `0x0034D998`, are loaded directly from their archived bytes with `DXT=0`.
  Those records already contain TMEM odd-row ordering, so conversion must undo
  the half swap to obtain a logical PS1 image.
- The PS1 converter therefore strips row padding everywhere, preserves XBMP
  atlas row order, and unswizzles standalone XOBF/XRTP rows. Applying either
  rule globally corrupts the other class.

The independent proofs are
`artifacts/n64_reference/dreamland-runtime-texture-layout-proof-20260814.json`
and
`artifacts/n64_reference/dreamland-runtime-texture-classes-proof-20260814.json`.
`verify_dreamland_terrain_atlas.py` now requires that runtime proof instead of
comparing conversion output only with the decoder that produced it.
...
decoded TIM payloads. XRTP's 12-byte descriptor layout is shared across the
ports, but its renderer flags are platform-specific. Dreamland's N64 primary
descriptor is `0x0192`; a runtime RDP capture ties its exact CI4 pixel payload
to opaque, Z-tested, Z-writing triangles. The converter therefore clears the
N64 `0x0100` bit before the descriptor reaches the PS1 primitive builder. Route
...
- XRTP `0x0100` belongs to PSX primitive semitransparency/blending, but the
  same bit in an N64 descriptor is not portable. Copying it into a converted
  PS1 file creates pale route strips and removes their depth writes. Native PS1
  files retain the bit unchanged; N64 conversion translates it. Transparent
  black remains per-texel alpha.
...
## N64 Super Dreamland 64 V8:2 Port

The V8:2 review port is isolated under `mods/v82_n64_super_dreamland` and is
generated by `tools/recompone-v8-2/port_dreamland_assets.py`. The converter:

- translates N64/V8 XOBF packet data into the V8:2 dialect;
- preserves the exact source hierarchy topology;
- replaces the 15-slot V8 shared-item bank with the native 24-slot V8:2 bank;
- translates TINF to XTIN and remaps shared-item object kinds;
- injects a native V8:2 XLSC trigger while presenting the decoded N64 loading
  JPEG; and
- extracts original V8 `SHELL/LOCATNS.BIN` root 12 for the selector preview.

The level overlay is generated by
`tools/recompone-v8-2/port_dreamland_overlay.py`. The converter statically
maps V8 object fields, hierarchy links, allocator/clone return aliases, engine
calls, globals, callbacks, and sound identifiers to the native V8:2 ABI.
Converted callbacks therefore run against ordinary V8:2 objects and memory;
there is no Dreamland-specific runtime memory view or model-preservation path.
Dynamic actor-model swaps and mushroom projectiles use the same native model
lifecycle and object constructors as retail V8:2 overlays.
