...
`tools/recompone-v8-2/port_dreamland_overlay.py`. V8 object fields are mapped
through `V82DreamlandCompat`, including the legacy primary callback at `+0x64`.
Dynamic actor-model swaps remain static because the V8 and V8:2 model-list
layouts cannot be mutated interchangeably. Mushroom projectiles allocate a
fresh V8:2 object instead of detaching and reallocating the legacy actor model.
