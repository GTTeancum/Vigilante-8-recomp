...
unconfirmed. Louisiana Ghastly Bayou is an explicit fail because the current
water edge remains visibly jagged; baseline comparison remains useful for
identifying the shared water-grid/shoreline/depth rule but no longer controls
the grade
...
| 2 | `LEVELS_BAYOU` | Louisiana Ghastly Bayou | https://www.youtube.com/watch?v=awgSqlSw8b4 | `01:21` sky/water/terrain/tree reference inspected; matching-view mist comparison requested | `route-gate-native-00-07-atmosphere-lifecycle-text`: exact current-hash span 1,594,937, path 2,374,432, 27 unique positions, exact HD card/terrain, pool drops 0, stale fog samples 0; current A047 focused proof `artifacts/v82-sheila-hoover-fix-20260902/sheila-bayou-proof/sheila-bayou-fixed.png` | **USER FAIL:** the current Dreamcast-parity renderer leaves a visibly jagged water edge. The correction must reproduce the shared water-grid/shoreline/depth-composition rule; no Louisiana, water-cell, polygon, vehicle, texture, packet-address, or authored-content workaround is permitted. The earlier hard-edged mist comparison and corrected Sheila attribution remain useful evidence but do not make this grade provisional. |
